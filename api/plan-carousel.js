...
async function reviewWithOpenAI(plan,researchContext){
  ...
  const response=await fetch("https://api.openai.com/v1/chat/completions",{method:"POST",headers:{Authorization:`Bearer ${process.env.OPENAI_API_KEY}`,"Content-Type":"application/json"},body:JSON.stringify({model,temperature:0.1,messages:[{role:"system",content:"당신은 광고 카드뉴스 편집장입니다. 사실성, 광고주 관점, 장표 반복, 후킹력, 디자인 템플릿 다양성, 회사소개 느낌, INWAVE 연결의 자연스러움을 엄격히 검수하세요. 연구 메모에 없는 수치가 있으면 high로 지적하세요. 최소 3종 템플릿이 아니거나 2장 이후 사진형이 반복되면 지적하세요."},{role:"user",content:`연구 메모:\n${researchContext?.researchText||"없음"}\n\n기획:\n${JSON.stringify(plan,null,2)}`}],response_format:{type:"json_schema",json_schema:{name:"carousel_review",strict:true,schema:REVIEW_SCHEMA}}})});
